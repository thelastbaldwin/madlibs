...
const madLib = [];
const nextOpeningTagIndex = (templateString, from = 0) => templateString.indexOf("{", from);
const nextClosingTagIndex = (templateString, from = 0) => templateString.indexOf("}", from);
...
while (stringIndex !== template.length){
    ...
    if (openingTagIndex > -1) {
        ...
        const replacementToken = readlineSync.question(`Enter a${isVowel(token[0]) ? "n" : ""} ${token.toLowerCase()}: `);
        // add everything from the last closing tag up, but not including, the next opening tag
